...
st.markdown(f"""
    <div class="header-clean">
        ...
        <p style="color:#00d4ff !important; font-weight:bold;">MUHAMMAD ANIQ IRFAN BIN MOHD ASMAZI</p>
    </div>
""", unsafe_allow_html=True)

# 8. LOGIK PEMETAAN & EXPORT
if uploaded_file:
    try:
        ...
        for i in range(len(df)):
            ...
            if show_bearing or show_distance:
                ...
                txt = f"{format_dms(brg) if show_bearing else ''} {'<br>' if show_bearing and show_distance else ''} {f'{dist:.2f}m' if show_distance else ''}"
                folium.Marker([mid_lat, mid_lon], icon=folium.DivIcon(html=f'<div style="font-size:8pt; color:yellow; font-weight:bold; text-shadow:1px 1px black;">{txt}</div>')).add_to(m)

        folium_static(m, width=1000)

        # BAHAGIAN EXPORT QGIS (GEOJSON)
